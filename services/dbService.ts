...
export const updateCase = async (id: string, caseData: Partial<Case>) => {
  ...
  try {
    ...
    // If document doesn't exist, don't create a new one - this is an update operation
    if (error instanceof Error && error.message.includes('No document to update')) {
      console.error(`❌ Case ${id} not found for update. Cannot create new case during update operation.`);
      throw new Error(`Case ${id} not found. Cannot update non-existent case.`);
    } else {
      throw error;
    }
  }
