...
class OfflineManager {
  ...
  async syncPendingActions(): Promise<void> {
    // Strong protection against multiple sync instances
    ...
    try {
      ...
      for (const action of actions) {
        try {
          ...
          // Update status immediately after each successful sync
          this.notifyStatusChange();
        } catch (error) {
          console.error(`Failed to sync action ${action.id}:`, error);
          action.retryCount = (action.retryCount || 0) + 1;
          
          // For document not found errors, don't retry - just remove action
          if (error.message?.includes('No document to update') || 
              error.message?.includes('No document to create') ||
              error.message?.includes('not found') ||
              error.code === 'not-found' ||
              error.code === 'permission-denied') {
            console.log(`🗑️ Removing failed action ${action.id} due to unrecoverable error: ${error.message}`);
            await this.removePendingAction(action.id);
            await this.logSyncAction(action, 'failed');
            // Update status immediately after removing failed action
            this.notifyStatusChange();
            ...
            await this.removePendingAction(action.id);
            await this.logSyncAction(action, 'failed');
            // Update status immediately after removing max retry action
            this.notifyStatusChange();
          }
        }
      }
      ...
      // Ensure syncInProgress is always reset
      this.syncInProgress = false;
      console.log('🔄 Sync finished, syncInProgress reset to false');
    }
  }
    ...
    switch (type) {
      ...
      case 'update':
        ...
        try {
          ...
          // If case doesn't exist, this is a critical error - don't create new case
          if (error instanceof Error && error.message.includes('not found')) {
            console.error(`❌ Cannot update case ${realCaseId}: Case not found in database`);
            throw new Error(`Case ${realCaseId} not found. Update operation failed.`);
          }
          throw error;
        }
        break;
