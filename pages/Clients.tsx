...
interface ClientsProps {
  ...
  onAddClientLocal?: (client: Client) => void;
...
const Clients: React.FC<ClientsProps> = ({ clients, cases, hearings, currentUser, onClientClick, onAddClient, onAddClientLocal, onUpdateClient, readOnly = false }) => {
  ...
  const saveClientToDatabase = async () => {
    if (!onAddClient || !formData.name || !currentUser?.firmId) {
      if (!currentUser?.firmId) {
    ...
    try {
      ...
      const newClient: Client = {
        ...
        firmId: currentUser.firmId // Use current user's firmId (must exist)
      };
      ...
      // If online AND no pending actions AND no sync in progress, add directly, otherwise queue for offline sync
      const isActuallyOnline = offlineStatus?.online && 
                              offlineStatus?.pendingActions === 0 && 
                              !offlineManager.isSyncInProgress();
      
      if (isActuallyOnline) {
        console.log('📡 Online mode: Adding client directly to Firebase');
        onAddClient(newClient);
      } else {
        // Queue for offline sync
        console.log('📴 Offline mode: Queuing client for sync (online:', offlineStatus?.online, 'pending:', offlineStatus?.pendingActions, 'syncInProgress:', offlineManager.isSyncInProgress(), ')');
        ...
        // Use onAddClientLocal to update UI without Firebase interaction
        if (onAddClientLocal) {
          onAddClientLocal(newClient);
        }
        
        console.log('📴 Client creation queued for offline sync:', newClient.name);
      }
      ...
      alert('حدث خطأ أثناء إنشاء الموكل. يرجى المحاولة مرة أخرى.');
    } finally {
    ...
    <div className="space-y-6 pb-20">
       ...
       <div className="bg-white dark:bg-slate-800 p-4 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 transition-colors">
          <div className="flex flex-col lg:flex-row justify-between items-center gap-4 mb-4">
             ...
             {offlineStatus && (
               <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg text-sm bg-slate-100 dark:bg-slate-700">
                 {offlineStatus.online ? (
                   ...
                   <>
                     ...
                     <span className="text-orange-600 dark:text-orange-400">غير متصل</span>
                   </>
                 )}
                 {offlineStatus.pendingActions > 0 && (
                   <span className="bg-orange-100 dark:bg-orange-900/30 text-orange-600 dark:text-orange-400 px-2 py-0.5 rounded-full text-xs font-medium">
                     {offlineStatus.pendingActions} إجراء في انتظار المزامنة
                   </span>
                 )}
               </div>
             )}
             ...
             <div className="flex gap-2 w-full lg:w-auto relative">
                ...
                {!readOnly && (
                  <button
                    ...
                    className="flex-1 lg:flex-none flex items-center gap-2 bg-primary-600 text-white px-4 py-2 rounded-lg hover:bg-primary-700 text-sm font-bold shadow-sm disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                     {isSubmitting ? (
                       <>
