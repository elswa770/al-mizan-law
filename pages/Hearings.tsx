...
const Hearings: React.FC<HearingsProps> = ({ hearings, cases, onCaseClick, onUpdateHearing, onAddHearing, readOnly = false }) => {
  ...
  const [decisionData, setDecisionData] = useState({
    ...
    rulingFiles: [] as Array<{name: string, url: string, driveId?: string}>, // Multiple files support
    completed: false
  });
  ...
  const openWizard = (hearing: Hearing) => {
    ...
    setDecisionData({
      ...
      rulingFiles: hearing.rulingFiles || [], // Load existing files
  ...
  const handleFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files.length > 0) {
      ...
      // Reset Google Drive state when new files are selected
      setUseGoogleDrive(false);
      
      if (useGoogleDrive) {
        // Upload to Google Drive first
        setIsUploadingToDrive(true);
        const uploadedFiles: Array<{name: string, url: string, driveId?: string}> = [];
        ...
        try {
          ...
          // Create folder name for hearing documents
          const caseInfo = getCaseDetails(editingHearing!.caseId);
          const folderName = `الجلسة - ${editingHearing!.date} - ${caseInfo?.title || 'قضية غير معروفة'}${caseInfo?.caseNumber ? ' - ' + caseInfo.caseNumber : ''}`;
          
          // Upload each file
          for (const file of files) {
            const response = await googleDriveService.uploadFile(file, folderName);
            uploadedFiles.push({
              name: file.name,
              url: response.webViewLink,
              driveId: response.fileId
            });
          }
          
          // Update decision data with all uploaded files
          setDecisionData(prev => ({
            ...prev,
            rulingFiles: [...prev.rulingFiles, ...uploadedFiles],
            rulingUrl: uploadedFiles[0]?.url || '', // Keep first file for backward compatibility
            rulingFileName: uploadedFiles.length > 0 ? `${uploadedFiles.length} ملفات` : ''
          }));
        ...
        // Update decision data with local files
        setDecisionData(prev => ({
          ...prev,
          rulingFiles: [...prev.rulingFiles, ...uploadedFiles],
          rulingUrl: uploadedFiles[0]?.url || '',
          rulingFileName: uploadedFiles.length > 0 ? `${uploadedFiles.length} ملفات (محلية)` : ''
        }));
      }
    }
  };
  ...
  const handleRemoveFile = (index: number) => {
    setDecisionData(prev => ({
      ...prev,
      rulingFiles: prev.rulingFiles.filter((_, i) => i !== index),
      rulingUrl: prev.rulingFiles.length > 1 ? prev.rulingFiles[0]?.url || '' : '',
      rulingFileName: prev.rulingFiles.length > 1 ? `${prev.rulingFiles.length - 1} ملفات` : ''
    }));
  };
  ...
  const handleWizardComplete = () => {
    if (editingHearing && onUpdateHearing) {
      ...
      onUpdateHearing({
        ...
        rulingFiles: decisionData.rulingFiles.length > 0 ? decisionData.rulingFiles : undefined,
                ...
                <div className="p-4 pr-5">
                   ...
                   <div className="flex justify-between items-start mb-3">
                      ...
                      <div className="flex gap-2 flex-wrap">
                         ...
                         {hearing.rulingFiles && hearing.rulingFiles.length > 0 ? (
                           hearing.rulingFiles.map((file, index) => (
                             <div key={index} className="flex items-center gap-1 bg-slate-100 dark:bg-slate-700 px-2 py-1 rounded-md">
                               <a 
                                 href={file.url} 
                                 target="_blank" 
                                 ...
                                 title={`عرض الملف: ${file.name}`}
                               >
                                 <FileText className="w-4 h-4" />
                                 <span className="text-xs font-medium truncate max-w-24">{file.name}</span>
                               </a>
                             </div>
                           ))
                         ) : hearing.rulingUrl ? (
                           // Fallback to single file if no rulingFiles
               ...
               <div className="p-6 min-h-[300px]">
                  ...
                  {wizardStep === 1 && (
                     <div className="space-y-4 animate-in fade-in slide-in-from-right-4">
                        ...
                        <div>
                           <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">مستندات الجلسة (يمكن رفع عدة ملفات)</label>
                           
                           {/* Google Drive Toggle */}
                           <div className="flex items-center justify-between p-3 bg-slate-50 dark:bg-slate-700/50 rounded-xl border border-slate-200 dark:border-slate-700 mb-4">
                           ...
                           {useGoogleDrive && !googleDriveService.isSignedIn() && (
                             <button
                               ...
                               className="w-full flex items-center justify-center gap-2 py-2 bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-600 rounded-lg text-sm font-bold text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors mb-4"
                             >
                           ...
                           <input 
                              ...
                              disabled={readOnly || isUploadingToDrive}
                              multiple
                           />
                           <div 
                              onClick={() => !readOnly && !isUploadingToDrive && rulingFileRef.current?.click()}
                              className={`border-2 border-dashed border-slate-300 dark:border-slate-600 rounded-xl p-6 text-center cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-700 hover:border-primary-400 transition-colors flex flex-col items-center gap-2 ${readOnly || isUploadingToDrive ? 'cursor-not-allowed opacity-60' : ''}`}
                           >
                              {isUploadingToDrive ? (
                                 <div className="flex items-center gap-3 text-blue-700 bg-blue-50 dark:bg-blue-900/20 dark:text-blue-400 px-4 py-2 rounded-lg">
                                    <div className="animate-spin w-5 h-5 border-2 border-blue-600 border-t-transparent rounded-full"></div>
                                    <span className="font-bold text-sm">جاري رفع الملفات إلى Google Drive...</span>
                                 </div>
                              ) : decisionData.rulingFiles.length > 0 ? (
                                 <div className="w-full space-y-2">
                                    <div className="flex items-center gap-3 text-green-700 bg-green-50 dark:bg-green-900/20 dark:text-green-400 px-4 py-2 rounded-lg">
                                       ...
                                       <span className="font-bold text-sm">تم إرفاق {decisionData.rulingFiles.length} ملفات</span>
                                    </div>
                                    <div className="space-y-1 max-h-32 overflow-y-auto">
                                       {decisionData.rulingFiles.map((file, index) => (
                                          <div key={index} className="flex items-center justify-between bg-white dark:bg-slate-700 px-3 py-2 rounded border border-slate-200 dark:border-slate-600">
                                 ...
                                 <>
                                    ...
                                    <p className="text-sm text-slate-500 dark:text-slate-400 font-medium">اضغط لرفع ملفات (صور أو PDF)</p>
                                    <p className="text-xs text-slate-400 dark:text-slate-500">يمكنك اختيار عدة ملفات في نفس الوقت</p>
                                    <p className="text-xs text-slate-400 dark:text-slate-500">
                                       {useGoogleDrive ? 'سيتم حفظها في Google Drive داخل مجلد الجلسة' : 'سيتم حفظها محلياً'}
                                    </p>
                                 </>
                              )}
                           </div>
