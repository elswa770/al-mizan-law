...
import { Calculator, Calendar, DollarSign, Percent, Info, RotateCcw, CheckSquare } from 'lucide-react';
...
const Calculators: React.FC<CalculatorsProps> = ({ onAddTask }) => {
  ...
  const [deadlineResult, setDeadlineResult] = useState<{ date: string; daysLeft: number; title: string } | null>(null);
  ...
  const calculateDeadline = () => {
    ...
    switch (procedureType) {
      case 'appeal_civil': 
        daysToAdd = 40; 
        title = 'استئناف حكم مدني/تجاري';
        break;
      case 'appeal_urgent': 
        daysToAdd = 15; 
        title = 'استئناف حكم مستعجل';
        break;
      case 'cassation': 
        daysToAdd = 60; 
        title = 'طعن بالنقض';
      ...
      case 'review': 
        ...
        break;
      case 'misdemeanor_appeal': 
        daysToAdd = 10; 
        title = 'استئناف حكم جنح';
        break;
      default: daysToAdd = 0;
    }
  ...
  const handleAddTaskFromDeadline = () => {
    if (!deadlineResult || !onAddTask) return;
    
    const newTask: Task = {
      id: Math.random().toString(36).substring(2, 9),
      title: `موعد نهائي: ${deadlineResult.title}`,
      description: `تذكير بانتهاء الميعاد القانوني للإجراء (${deadlineResult.title}) المحسوب بناءً على تاريخ الحكم ${judgmentDate}.`,
      dueDate: deadlineResult.date,
      priority: 'high',
      status: 'pending',
    };
    
    onAddTask(newTask);
    alert('تمت إضافة المهمة بنجاح إلى قائمة المهام');
  };
    ...
    <div className="space-y-6 animate-in fade-in">
      ...
      {deadlineResult && (
        <div className="mt-6 p-6 bg-slate-50 dark:bg-slate-700/50 rounded-xl border border-slate-200 dark:border-slate-600 text-center">
          ...
          {onAddTask && deadlineResult.daysLeft > 0 && (
            <button 
              onClick={handleAddTaskFromDeadline}
              className="mt-4 w-full py-2 bg-indigo-600 text-white rounded-lg font-bold hover:bg-indigo-700 flex items-center justify-center gap-2"
            >
              <CheckSquare className="w-4 h-4" /> إضافة تذكير للمهام
            </button>
          )}
        </div>
      )}
