...
interface CasesProps {
  ...
  onAddCaseLocal?: (newCase: Case) => void;
  readOnly?: boolean;
}

const Cases: React.FC<CasesProps> = ({ cases, clients, lawyers = [], currentUser, onCaseClick, onAddCase, onAddCaseLocal, readOnly = false }) => {
  ...
  const handleAddSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isActuallyOnline) {
        console.log('📡 Online mode: Adding case directly to Firebase');
        onAddCase(newCase);
      } else {
        // Queue for offline sync
        console.log('📴 Offline mode: Queuing case for sync (online:', offlineStatus?.online, 'pending:', offlineStatus?.pendingActions, 'syncInProgress:', offlineManager.isSyncInProgress(), ')');
        ...
        // Use onAddCaseLocal to update UI without Firebase interaction
        if (onAddCaseLocal) {
          onAddCaseLocal(newCase);
        }
        
        console.log('📴 Case creation queued for offline sync:', newCase.title);
      }
  ...
  const getLawyerName = (id?: string) => {
    ...
    const user = lawyers?.find(u => u.id === id);
      ...
      <div className={`mb-4 p-3 rounded-lg border flex items-center gap-3 ${
        ...
        <div className="flex-1">
          <p className={`text-sm font-medium ${
            ...
            {offlineStatus.online ? 'متصل بالإنترنت' : 'وضع الأوفلاين'}
          </p>
          {offlineStatus.pendingActions > 0 && (
            <p className="text-xs text-amber-600 dark:text-amber-400">
              {offlineStatus.pendingActions} إجراءات في انتظار المزامنة
            </p>
          )}
        </div>
        {!offlineStatus.online && (
            ...
            <form onSubmit={handleAddSubmit} className="p-6 space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                   ...
                   <select className="w-full border p-2 rounded-lg bg-white dark:bg-slate-700 dark:border-slate-600 dark:text-white" value={formData.assignedLawyerId || ''} onChange={e => setFormData({...formData, assignedLawyerId: e.target.value})}>
