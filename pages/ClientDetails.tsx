...
import { ArrowRight, User, Phone, MapPin, Mail, FileText, Calendar, Briefcase, Hash, Save, X, ScrollText, AlertTriangle, Upload, Eye, CheckCircle, Trash2, Edit3, Plus, File, Building2, Wallet, BellRing, PhoneCall, MessageCircle, MoreVertical, Clock, Send, Copy, Cloud, LogIn, Wifi, WifiOff } from 'lucide-react';
...
const ClientDetails: React.FC<ClientDetailsProps> = ({ clientId, clients, cases, hearings = [], currentUser, onBack, onCaseClick, onUpdateClient, readOnly = false }) => {
  ...
  const handleSaveClient = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!onUpdateClient || !formData.name) return;
    
    setIsSubmitting(true);
    
    try {
      const updatedClient = { ...client, ...formData } as Client;
      ...
      if (offlineStatus?.online) {
        ...
        console.log('📴 Client update queued for offline sync:', updatedClient.name);
      }
  ...
  const handleSaveDocument = async () => {
     if (onUpdateClient && selectedFile && newDocData.name) {
        ...
        if (useGoogleDrive) {
          ...
          try {
            const folderName = `الموكل - ${client.name}`;
           ...
           <div className="px-6 pb-6 -mt-10 relative">
              <div className="flex justify-between items-end mb-4">
                 ...
                 <div className="flex flex-col items-end gap-2">
                   ...
                   {offlineStatus && (
                     <div className="flex items-center gap-1.5 px-2 py-1 rounded-lg text-xs bg-slate-100 dark:bg-slate-700">
                       ...
                       {offlineStatus.pendingActions > 0 && (
                         <span className="bg-orange-100 dark:bg-orange-900/30 text-orange-600 dark:text-orange-400 px-1.5 py-0.5 rounded-full text-xs font-medium">
