...
import { 
  ...
  Plus, Edit3, Trash2, Clock, Mail, FileSpreadsheet, File as FileIcon, PenTool, CheckCircle, ArrowRight
} from 'lucide-react';
...
const Reports: React.FC<ReportsProps> = ({ cases, clients, hearings, tasks }) => {
          ...
          <div key={template.id} className="bg-white dark:bg-slate-800 p-6 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 hover:shadow-md transition-all group relative">
            ...
            <button 
              onClick={() => {
                ...
                setActiveTab('generated' as any);
              }}
              className="w-full py-2 border border-indigo-200 dark:border-indigo-800 text-indigo-600 dark:text-indigo-400 rounded-lg text-sm font-bold hover:bg-indigo-50 dark:hover:bg-indigo-900/30 transition-colors"
            >
              استخدام القالب
            </button>
          </div>
  ...
  const renderGeneratedReport = () => {
    if (!activeTemplate) return <div className="text-center p-8 text-slate-500">لم يتم اختيار قالب</div>;
      ...
      <div className="space-y-8 animate-in fade-in">
        <div className="flex justify-between items-center bg-white dark:bg-slate-800 p-4 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700">
           <div>
              ...
              <p className="text-sm text-slate-500 dark:text-slate-400">تم إنشاء هذا التقرير بناءً على القالب المخصص</p>
           </div>
           <button 
             onClick={() => setActiveTab('builder')}
             className="text-sm text-slate-500 hover:text-slate-800 dark:hover:text-white font-bold flex items-center gap-1"
           >
              <ArrowRight className="w-4 h-4" /> العودة للمصمم
           </button>
        </div>

        {activeTemplate.sections.includes('cases') && (
           <div className="space-y-4">
              <h4 className="font-bold text-slate-700 dark:text-slate-300 border-b pb-2">ملخص القضايا</h4>
              {renderOverviewTab()}
           </div>
        )}

        {activeTemplate.sections.includes('financial') && (
           <div className="space-y-4">
              <h4 className="font-bold text-slate-700 dark:text-slate-300 border-b pb-2">التقرير المالي</h4>
              {renderFinancialTab()}
           </div>
        )}

        {activeTemplate.sections.includes('operational') && (
           <div className="space-y-4">
              <h4 className="font-bold text-slate-700 dark:text-slate-300 border-b pb-2">الأداء التشغيلي</h4>
    ...
    <div className="space-y-6 pb-20 print:p-0 print:m-0">
       ...
       <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-4 bg-white dark:bg-slate-800 p-4 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 no-print">
          ...
          <div className="flex flex-wrap items-center gap-2">
             <div className="bg-slate-100 dark:bg-slate-700 p-1 rounded-lg flex text-xs font-bold overflow-x-auto max-w-full">
                ...
                {activeTemplate && (
                   <button 
                     onClick={() => setActiveTab('generated' as any)}
                     className={`px-3 py-1.5 rounded-md transition-all whitespace-nowrap ${activeTab === 'generated' ? 'bg-white dark:bg-slate-600 text-indigo-700 dark:text-indigo-400 shadow' : 'text-slate-500 dark:text-slate-400'}`}
                   >
                      التقرير المولد
                   </button>
                )}
       ...
       <div className="hidden print:block mb-8 text-center border-b border-slate-300 pb-4">
          ...
          <p className="text-sm text-slate-600 mt-1">
             {activeTab === 'overview' ? 'نظرة عامة شاملة' : activeTab === 'financial' ? 'تقرير الوضع المالي' : activeTab === 'operational' ? 'تقرير الأداء التشغيلي' : activeTab === 'generated' ? `تقرير مخصص: ${activeTemplate?.name}` : ''}
          </p>
       </div>
       ...
       <div className="min-h-[500px]" id="report-content" ref={reportRef}>
          ...
          {activeTab === 'generated' && renderGeneratedReport()}
          
          {/* Signature Footer */}
          {signature && (activeTab === 'overview' || activeTab === 'financial' || activeTab === 'operational' || activeTab === 'generated') && (
